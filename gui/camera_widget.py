...
class CameraWidget(QFrame):
    ...
    def _render_hud_overlay(self, image: np.ndarray, info: dict) -> None:
        ...
        if card_y + card_h < h and card_x + card_w < w:
            ...
            # Text lines
            cv2.putText(image, f"FPS: {fps:.1f}", (card_x + 12, card_y + 24), font, 0.45, (0, 230, 0), 1, cv2.LINE_AA)
            ...
            cv2.putText(image, f"Steering: {angle:+.1f} deg", (card_x + 12, card_y + 68), font, 0.45, (255, 229, 0), 1, cv2.LINE_AA)
    ...
    def show_offline_banner(self, message: str = "Camera Offline") -> None:
        ...
        self.lbl_image.setText(
            f"📷  {message}\n\nPress [START PIPELINE] to launch live gesture controls."
        )
        self.lbl_image.setStyleSheet(
            "background-color: #0d0e12; color: #5b616e; font-size: 16px; "
            "font-weight: bold; border-radius: 10px; text-align: center;"
        )
