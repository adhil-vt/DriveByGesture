"""
tests.unit.test_peace
=====================
Unit tests for PeaceGesture implementation.
"""
...
class TestPeaceGesture(unittest.TestCase):
    def setUp(self):
        self.gesture = PeaceGesture()
    ...
    def test_peace_gesture_thumb_extended_or_partially_bent(self):
        # Thumb position should not determine Peace gesture success
        hand_ext = make_hand_analysis(thumb_pos=FingerPosition.EXTENDED)
        hand_bent = make_hand_analysis(thumb_pos=FingerPosition.PARTIALLY_BENT)

        res_ext = self.gesture.recognize(hand_ext)
        res_bent = self.gesture.recognize(hand_bent)

        self.assertTrue(res_ext.detected)
        self.assertTrue(res_bent.detected)

    def test_slightly_relaxed_peace_index_or_middle_partially_bent(self):
        hand = make_hand_analysis(
    ...
    def test_slightly_relaxed_peace_ring_or_pinky_partially_bent(self):
        hand = make_hand_analysis(
            index_pos=FingerPosition.EXTENDED,
            middle_pos=FingerPosition.EXTENDED,
    ...
    def test_pinch_must_fail(self):
        # Pinch pose: Ring and Pinky are extended or curled while Index/Middle are curled or in pinch state
        hand = make_hand_analysis(
            thumb_pos=FingerPosition.EXTENDED,
            index_pos=FingerPosition.EXTENDED,
            middle_pos=FingerPosition.CURLED,
